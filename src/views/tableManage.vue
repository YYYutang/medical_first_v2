...
export default {
  ...
  methods: {
    ...
    append() {
      ...
      addDisease("/api/addFeature2", catagoryNodeJSON).then(response => { // 后端没找到这个接口
    ...
    appendDisease() {
      ...
      let catagoryNodeJSON = JSON.stringify(catagoryNode);
      addDisease("/api/appendDisease2", catagoryNodeJSON).then(response => { // 后端没找到这个接口
        this.getCatgory(); //刷新目录结构
        console.log(response.data);
      }).catch(error => {
        alert("新增疾病目录错误" + error)
    ...
    changeData(...theArgs) {
      ...
      if (theArgs[0].isLeafs == 1) {
        postRequest("/api/filed/getAllFiled", { diseaseName: disease }).then((res) => {
          console.log(res);
